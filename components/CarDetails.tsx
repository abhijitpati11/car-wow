"use client";

import { CarProps } from '@/types';
import Image from 'next/image'
import { Fragment } from 'react';

import { Dialog, Transition } from '@headlessui/react';
import { generateCarImageUrl } from '@/utils';

interface carDetailsProps {
 isOpen: boolean;
 closeModal: () => void;
 car: CarProps;
}

const CarDetails = ({ isOpen, closeModal, car }: carDetailsProps) => {
  return (
    <>
     <Transition appear show={isOpen} as={Fragment}>
       <Dialog as='div' className='relative z-10' onClose={closeModal}>
        <Transition.Child
         as={Fragment} 
         enter='ease-out duration-300'
         enterFrom='opacity-0'
         enterTo='opacity-100'
         leave="ease-in duration-200"
         leaveFrom='opacity-100'
         leaveTo='opacity-0'
        >
         <div className='fixed inset-0 bg-black bg-opacity-25' />
        </Transition.Child>

        <div className='fixed inset-0 overflow-y-auto'>
         <div className='flex min-h-full items-center justify-center p-4 text-center'>
         <Transition.Child
          as={Fragment} 
          enter='ease-out duration-300'
          enterFrom='opacity-0 scale-95'
          enterTo='opacity-100 scale-100'
          leave="ease-in duration-200"
          leaveFrom='opacity-100 scale-100'
          leaveTo='opacity-0 scale-95'
         >
          <Dialog.Panel className='relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white text-left shadow-xl transition-all flex flex-col gap-5 p-6'>
            <button
             type='button'
             onClick={closeModal}
             className='absolute top-2 right-2 z-10 w-fit p-2 bg-primary-blue-100 rounded-full hover:bg-red-500'
            >
             <Image 
              src="/close.svg"
              alt='close'
              width={20}
              height={20}
              className='object-contain'
             />
            </button>

            {/* details of the cars in the modal */}
            <div className='flex-1 flex flex-col gap-3'>
             {/* main image of the car */}
             <div className='relative w-full h-40 bg-pattern bg-cover bg-center rounded-lg'>
               <Image src={generateCarImageUrl(car)} alt="hero" fill priority className='object-contain' />
             </div>

             {/* top and side view of the car */}
             <div className='flex gap-3'>
              <div className='flex-1 relative w-full h-20 bg-primary-blue-100 rounded-lg'>
                <Image src={generateCarImageUrl(car, '29')} alt="hero" fill priority className='object-contain' />
              </div>
              <div className='flex-1 relative w-full h-20 bg-primary-blue-100 rounded-lg'>
                <Image src={generateCarImageUrl(car, '33')} alt="hero" fill priority className='object-contain' />
              </div>
              <div className='flex-1 relative w-full h-20 bg-primary-blue-100 rounded-lg'>
                <Image src={generateCarImageUrl(car, '13')} alt="hero" fill priority className='object-contain' />
              </div>
             </div>
            </div>

            {/* car specifications */}
            <div className='flex-1 gap-2 flex flex-col cursor-pointer'>
              <h2 className='font-semibold text-xl capitalize pl-3'>
                {car.make} {car.model}
              </h2>

              <div className='mt-3 flex flex-wrap gap-4'>
                {Object.entries(car).map(([key, values]) => (
                  <div className='flex justify-between gap-5 w-full text-right px-3 hover:border hover:bg-green-200 p-2 hover:border-r-4' key={key}>
                    <h4>{key.split("_").join(" ")}</h4>
                    <p className='font-semibold'>{values}</p>
                  </div>
                ))}
              </div>
            </div>
            
          </Dialog.Panel>
        </Transition.Child>
         </div>
        </div>

       </Dialog>
     </Transition>
    </>
  )
}

export default CarDetails
